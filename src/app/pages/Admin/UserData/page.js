...
export default function AdminUserData() {
  ...
  const deleteUser = async (id) => {
    try {
      await axios.delete(`https://api.marasimpex.com/api/userdatas/${id}`);
      ...
      <div className="flex-1 flex flex-col overflow-auto">
        ...
        <div className="p-4 overflow-x-auto">
          <table className="min-w-full bg-white border border-gray-300 shadow-lg rounded-lg">
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user._id} className="text-gray-700 border-t border-gray-200">
                  <td className="py-3 px-6">{user.name}</td>
                  <td className="py-3 px-6">{user.email}</td>
                  <td className="py-3 px-6">{user.phone}</td>
                  <td className="py-3 px-6">{user.address}</td>
                  <td className="py-3 px-6">{user.city}</td>
                  <td className="py-3 px-6">{user.pin}</td>
